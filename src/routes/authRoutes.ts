...
router.get('/status', (req: Request, res: Response) => {
    ...
    if (req.isAuthenticated && req.isAuthenticated()) {
        ...
        console.log(`[Auth Status]: User is authenticated. User ID: ${req.user?.id}`); // Use optional chaining on user
        res.status(200).json({
...
router.post('/logout', (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.logout) {
         ...
         req.logout((err?: Error) => {
             if (err) {
                 console.error("[Logout Error - req.logout]: ", err);
             ...
             req.session.destroy((destroyErr) => {
                 ...
                 console.log(`[Logout]: Clearing cookie: ${sessionCookieName}`);
         ...
         console.warn("[Logout]: req.logout function not found, attempting session destroy only.");
         req.session.destroy((destroyErr) => {
             if (destroyErr) {
                 console.error("[Session Destroy Error - Fallback]: ", destroyErr);
                 return next(destroyErr);
             }
             const sessionCookieName = process.env.SESSION_NAME || 'connect.sid';
             console.log(`[Logout]: Clearing cookie (fallback): ${sessionCookieName}`);
             res.clearCookie(sessionCookieName);
