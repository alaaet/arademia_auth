...
router.get('/:uid', setNoCache, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        logger.info(`Interaction check for UID: ${uid}, Prompt: ${prompt.name}`);
        // Log session details (if available) to see if user is already logged in at this stage
        if(session) {
            logger.info(`Session details: accountId=${session.accountId}, uid=${session.uid}, exp=${exp}`);
        } else {
             logger.info('No active session found for this interaction.');
        }
        ...
        // Validate client
        if (!client) {
            logger.error(`Interaction client not found for client_id: ${params.client_id} (UID: ${uid})`);
        ...
        switch (prompt.name) {
            ...
            case 'consent': {
                ...
                logger.info(`Consent prompt details for UID ${uid}:`, JSON.stringify(prompt.details, null, 2));
                // --- End Debugging ---
                ...
                if (!accountId) {
                   ...
                   logger.error(`Account ID missing for consent prompt. Session: ${JSON.stringify(session)}, Prompt Details: ${JSON.stringify(prompt.details)}`);
                   return next(new Error('Account ID missing for consent prompt')); // Error reported by user
                }
                logger.info(`Account ID found for consent: ${accountId}`);

                const account = await Account.findAccount(undefined, accountId as string); // Find account details
                if (!account) {
                   logger.error(`Account object not found for accountId: ${accountId}`);
                ...
                logger.info(`Scopes requiring consent: ${scopesToDisplay.join(', ')}`);
...
router.post('/:uid/login', setNoCache, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        logger.info(`Login successful for accountId: ${accountIdString}`);
        const result = {
            login: {
...
router.post('/:uid/confirm', setNoCache, async (req: Request, res: Response, next: NextFunction) => {
    logger.info(`[Interaction /confirm POST] Received consent confirmation for UID: ${req.params.uid}`); // Log entry
    try {
        ...
        logger.info('[Interaction /confirm POST] Updating grant scopes/claims...');
        if (details.missingOIDCScope) {
             ...
             logger.info('[Interaction /confirm POST] No missing OIDC scopes.');
        }
        ...
        logger.info('Grant object before save:', JSON.stringify(grant, null, 2));
        ...
        try {
            ...
            // ADD THIS LOG:
            logger.info('[Interaction /confirm POST] interactionFinished resolved with:', finishResult);
            logger.info('[Interaction /confirm POST] Provider should now redirect.');
        } catch (err) {
            logger.error('[Interaction /confirm POST] Error during or after interactionFinished:', err);
            next(err);
        }
        // await oidcProvider.interactionFinished(req, res, consentResult, { mergeWithLastSubmission: false });
        // logger.info('[Interaction /confirm POST] interactionFinished completed. Provider should now redirect.');
        ...
        // If the browser hangs, check the Network Tab for the response to this POST.
         ...
         logger.error('[Interaction /confirm POST] Error caught:', err);
        next(err);
    }
});
